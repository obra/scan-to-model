...
Output `measurements.json` records exact component hashes, variant selection, native sample support, transforms, plane parameters, all-sample and retained-sample residuals, comparison overlap, and review figure names. A successful plane also reports `retained_support_spans_m` (the two max-min spans projected onto the fitted SVD in-plane axes) and `retained_centered_singular_values_m` (the three singular values of retained points centered at their retained mean). These describe support geometry in metres and do not add a threshold or pass/fail claim. Per-patch NPZs contain `raw_points`, transformed `points`, native depth `pixels` in `(u,v)` order, and `inlier_mask`. JPEGs show the selected photo polygon beside the native filtered depth. A low plane residual does not establish sensor accuracy. Interpret the physical surface before comparing it; a ramp and a flat floor cannot be treated as repeat observations of one plane.

Each patch also reports `native_depth_counts` and `native_confidence_histogram` over its selected native mask. `in_range` means a positive depth no greater than `max_depth_m`; `out_of_range` includes zero and values beyond that limit. These are source-encoding and filter counts, not physical-support or accuracy claims. When `--pixel-inspections` is enabled, `pixel_inspection.eligible_samples` records the filtered native `(u,v)` coordinates, their RGB-native and displayed centers, and the existing resolution/orientation mapping used to mark them in the source crop. That mapping uses NumPy `rint` (ties-to-even) before clipping to RGB bounds. The inspection remains pending visual review and does not alter NPZ samples or fit results.
...
For provisional photo overlays, `python3 -B scripts/preview_geometry.py --spec <spec.json> --output <new-output>` accepts finite model-space `vertices`, indexed `faces`, and `frames` with native RGB/camera paths, `raw_to_model4x4`, and display orientation. It rotates native RGB exactly once for `upright90cw`, writes an exact OBJ and per-frame projected pixels, depths, front flags, and clipped wire overlays, and adds a note that wires are provisional and can include occluded geometry; behind-camera faces are recorded as skipped.

`geometry.aperture_lining(tangent, normal, plane_constant, u_bounds, z_bounds, depth)` emits only the two recessed jambs and recessed head of a rectangular opening. Depth is positive and recedes along `-normal`; the helper shares eight vertices and emits no sill, back, or front cap.

Use `scripts/run_surfaces.py` for repeatable surface runs. It copies `surfaces.py`, `polycam.py`, and `pixel_inspection.py` into a per-run runtime, resolves relative top-level and per-patch capture paths against the original spec directory, and records the original/resolved specs, helper hashes, full subprocess argv/cwd, logs, and output-file hashes. The output directory must be new; failed source availability still leaves a failure receipt. For example: `python3 -B scripts/run_surfaces.py --spec <project>/docs/surfaces.json --output <project>/derived/surface-review-01 --pixel-inspections`. The receipt is provenance for the run, while native sample support remains subject to the surface and source-review limits below. For upright overlays, use `polycam.project_display_points` so native projection pixels are converted exactly once to the declared display orientation. For model-space points, use `polycam.project_model_points` with the accepted raw-to-model affine map; it applies the inverse map once before the raw camera projection.
...
Room view subjects use `edge_mode: "existing_edges"` by default, which emits each selected polygon's edges. Non-section views may set `edge_mode: "coplanar_boundary"` to draw the boundary of a selected coplanar patch after spatial and plan clipping. The drawing helper cancels only shared edge segments used by two selected pieces; it preserves boundaries of concave and disjoint pieces and rejects non-coplanar or non-manifold selections. Coplanarity allows up to one micron of point-distance noise from float32 evaluation; edge matching retains its tighter topology tolerance. It assumes a finite, non-overlapping patch with valid edge valences; it does not perform a full polygon union. This mode is rejected for sections, whose cut geometry has separate semantics. Manifest `drawn_faces` includes only selected faces with positive emitted projected line length inside the declared view bounds; wholly off-page subjects retain their emitted geometry but are listed in `omitted_objects` with an out-of-bounds reason.

Plan `section_marks` must reference one section view by `section_id`; their axis/value must match that view's cut, and the arrow segment must be finite, nonzero, parallel to and directed along the section view's plan direction. `section_id` supplies the stable section identity; `label` is display text.
