...
Use a Python 3.11+ environment with the packages in `requirements.txt`. In these examples `$PLUGIN` is the absolute installed plugin root, `$PROJECT` is the project directory, and `$CAPTURE` is an extracted directory containing `keyframes/`. Quote paths. The `-B` flag keeps these read-and-write CLI examples from creating `__pycache__` files in the installed plugin. Investigate CLI failures before continuing. Do not edit raw inputs to make a tool pass. If a helper needs package-local imports, run it from a source checkout or another writable project environment; do not make the installed plugin cache its import location.

## Preserve and inspect a capture

```sh
python3 -B "$PLUGIN/scripts/ingest.py" /path/to/capture.zip --project "$PROJECT" --capture-id survey-one
python3 -B "$PLUGIN/scripts/reference.py" "$CAPTURE" "$PROJECT/derived/reference-one.npz" --frame-step 20 --pixel-step 4 --depth-variant raw --pose-variant raw
```
...
```sh
python3 -B "$PLUGIN/scripts/surfaces.py" --spec "$PROJECT/docs/surfaces.json" --output "$PROJECT/derived/surface-review"
```
...
```sh
python3 -B "$PLUGIN/scripts/register.py" --spec "$PROJECT/docs/landmarks.json" --output "$PROJECT/derived/registration.json"
```
