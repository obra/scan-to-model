...
For provisional photo overlays, `python3 -B scripts/preview_geometry.py --spec <spec.json> --output <new-output>` accepts finite model-space `vertices`, indexed `faces`, and `frames` with camera/RGB paths, `raw_to_model4x4`, and display orientation. It writes an exact OBJ and per-frame projected pixels, depths, front flags, and clipped wire overlays; behind-camera faces are recorded as skipped.
