...
Use `scripts/run_surfaces.py` for repeatable surface runs. It copies `surfaces.py`, `polycam.py`, and `pixel_inspection.py` into a per-run runtime, resolves relative top-level and per-patch capture paths against the original spec directory, and records the original/resolved specs, helper hashes, full subprocess argv/cwd, logs, and output-file hashes. The output directory must be new; failed source availability still leaves a failure receipt. For example: `python3 -B scripts/run_surfaces.py --spec <project>/docs/surfaces.json --output <project>/derived/surface-review-01 --pixel-inspections`. The receipt is provenance for the run, while native sample support remains subject to the surface and source-review limits below.
