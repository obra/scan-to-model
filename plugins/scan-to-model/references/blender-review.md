...
For requested photographic appearance, rendered still tours, packed native/GLB files and offline web fly-throughs, use the shared producer in [delivery.md](delivery.md). It performs early mesh checks, actual whole-scene object/material raster review, native reopen checks and copied-directory browser tests. Reuse those checks when their inputs are unchanged; the inventory helper below remains useful for geometry/source review. Neither producer makes an inference into a measured fact or adds an approval loop. An owner's retention request overrides defaults for retiring intermediate artifacts.
