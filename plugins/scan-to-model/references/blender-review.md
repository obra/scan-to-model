...
## Scope a reconstruction review

Review one coherent model increment and its affected interfaces. Bind the current model and relevant accepted source/placement records once. Reuse the baseline inventory only when the exact model and inventory-helper hashes match; the identity requirements below still apply. Compare the candidate's declared additions/changes and inspect views that reveal its shape, openings, materials and contacts. A local change does not require replaying unchanged source analyses or rerendering unrelated sheets. A change to a shared transform, material, visibility rule or runtime requires checks for its affected consumers.

For offline work, exact source-authoring or older readback geometry can support qualified preparation when `scripts/mesh_fingerprint.py` coordinate/polygon hashes and the declared frame, parent, constraint and modifier scope match the current inventory. Check the evaluated pose: even an unparented object's unevaluated `matrix_world` can be stale. Compare original and evaluated topology counts together with modifiers; a count difference alone does not establish a stale model. Saved records do not establish current native contact or appearance beyond their scope.

Use `scripts/blender_mesh.py` for explicit world-space mesh replacement and `geometry.panel_quad` for regular planar faces. Apply the source-to-model transform once before deriving extents, voids or attached parts. A perforated panel is its filled surface minus real apertures, not outline curves; reuse accepted surface polygons with true holes when applicable. Read back exported geometry and compare vertices and winding with the reviewed plan. Reprojection evidence comes from the actual geometry through the maintained helper, not copied input pixels. Resolve actual shader inputs, UV mapping and created datablock names in the saved candidate when appearance changes.

For room extraction and drawings, resolve native membership from accepted feature/object IDs and semantic room tags, including shared collections. Compare the resolved set with the declared set. On collection or view changes, check affected room/level view intents and before/after visible sets; mixed-level collections may require scoped layer hides. Use the maintained helper's equality-based RNA selection and enable the intended view layer. Call `validate_view_frame` for proposed drawing views and separately establish the physical inside/outside viewpoint; handedness alone does not identify the building side.

Shader reviews use saved native objects and materials. For selected assembly views, a disposable scene may link those objects and parent ancestors into one review view layer, or use the helper's explicit single-layer control. Frame cameras from subject bounds. Compare all declared datablock categories; match scene identities and list counts before pairing records. Undeclared additions, changes or removals fail preservation review.

Use maintained runners to freeze executed producers, inputs and runtimes, including failed runs. An archived producer may write beside itself: inspect its output behavior before any necessary replay and direct that replay to fresh outputs. Record unavailable failed-producer bytes as a provenance gap. Refer to existing unchanged retained inputs rather than creating another copy for a handoff. Do not rerun a saved preparation simply to reproduce its receipt; replay is warranted by a changed producer, failed result or unresolved reproducibility concern.
...
Before launch, resolve every render `target_names` entry to an actual current object name or an explicitly resolved stable ID; a declared missing target must fail rather than silently disappear.

The contract's renderer section declares `allowed_engines` (`BLENDER_EEVEE` and/or `CYCLES`), `material_mode: "materials-and-textures"`, required material and image IDs, and a `material_overrides` list, which may be empty. Required IDs are the materials and images deliberately chosen to appear in the union of first-surface sample reports. Each material override records the current-eligible object ID, original material ID, render material ID, and the physical appearance intent. The run receipt repeats the exact override list. This permits a narrowly audited render-only material correction without confusing it with native material preservation. An applied override can remain outside the required visible set when another surface occludes it; adding its render material ID to the required set makes absence from the samples a validation failure.

Derive required image identities from current evaluated surface material assignments and image nodes reachable from the active shader outputs, rather than from datablock or material-slot presence alone. Preserve unused or source-evidence images separately, and reconcile replacement materials and images against actual face assignments and raster material/image evidence before rendering more views.
...
Before editing a camera contract, inspect the worker's target schema and the native `target_center` resolver. Use the exact structured target form the worker consumes: when a static aim requires two target records, provide both records explicitly, with their object-ID groups, rather than a shorthand or an ignored scalar such as `target_m`. Resolve the target objects and report their bounds and computed center; stop if the center is empty, non-finite, or outside the intended physical subjects. Treat broad collections such as contents or work areas as candidates only: verify their resolved object bounds against the room or connection scope before including them. Give each run a newly created, task-owned runtime root and verify that the launch plan points to it.
...
When assembling opaque wall cells into a planar union, split shared edges at every T-junction or use a true polygon-union operation before tracing boundaries. Classify a rectangular profile after removing collinear subdivision vertices while retaining the original boundary vertices in the evidence packet. Check the union area against an independently enumerated occupied-cell area, and sort room extents by their physical coordinate before deriving inter-room gaps; alphabetical order does not describe spatial adjacency.

Before reporting a source-to-current residual, resolve the exact current object and face through the current authoring or selection records and verify its recorded physical role. A matching frame or source ID can identify retained raw or diagnostic geometry rather than the selected architectural finish. Default visibility does not establish that role because valid selected geometry can be excluded from the active view. When structured records are returned, inspect their declared source and registration fields for the exact source ID before reporting a missing bridge; the matching record may be nested under a different property.

Comparing a raw source-derived mesh with a plane fitted from that same source verifies preservation or derivation, not an independent measurement of current geometry. Report historical source-to-source residuals and source-to-current selected-face residuals in separate columns. For each column, name the two compared surfaces, the common footprint where both have support, and the applicable residual limit; do not combine results across different support regions.

Before launching Blender for a finite face comparison, project documented support footprints whose exact revision and selected-face roles apply to the intended comparison. Bind each footprint to the predecessor or current artifact that supplied it; do not substitute an older layout extent for current vertices or present the screen as a current native read. If applicable documented footprints are disjoint, stop without a native run or residual. Stale or unknown predecessor bounds do not establish current disjointness and can require a bounded current read. If a native read proceeds, intersect the actual projected face polygons, including their vertical extent, and report no comparison when their common area is empty.

An evaluated object bound or historical full rectangle is only a screening extent; it does not prove that the current mesh covers that footprint. Ownership clipping can leave complementary stair-step fragments across adjoining room objects. Zero overlap with one retained fragment does not establish that the intended interface lacks a current surface. Resolve the adjoining current owners and intersect their evaluated mesh polygons in the declared face frame before making a claim about the complete intended interface, and retain that owner mapping in the comparison record. Absence from the current model is a model-coverage result; do not infer that the physical wall is absent without separate physical evidence.

Polygon vertex winding is an ordering convention and does not identify the physical or source-facing side. Record the declared physical side separately, derive or check each face normal from the current evaluated polygon in the declared coordinate frame, and document any intentional winding reversal.

Use one declared orthonormal basis for both projection and reconstruction. Orient the common normal toward the declared side, derive perpendicular along-wall and upward axes, and reconstruct with those same axes; literal world Z is not the height coordinate for a tilted face. Compute the angle between face normals as `atan2(norm(cross(normal_a, normal_b)), dot(normal_a, normal_b))` so nearly parallel normals remain stable. Test a tilted parallel-face case with a known gap and overlap area, and round-trip every returned world vertex through the reported basis. When a test file supports both `python -m unittest` and direct execution, run both entrypoints and place `unittest.main()` after all test classes so direct execution includes the complete suite.
...
Its manifest separates per-view `nondrawable_objects` for exact selected point collapse from `omitted_objects` caused by projection, cuts, clipping or declared bounds. `drawn_faces` includes only selected faces with positive emitted projected line length inside the declared view bounds; point collapse is a geometry diagnostic, not proof of physical absence, so coverage reviewers still inspect the pixels.

Every declared view must provide a unit `view_direction_frame` alongside its unit `right_frame` and `up_frame`. The producer checks `right_frame = cross(view_direction_frame, up_frame)`; a `reflected_ceiling` view may instead set `horizontal_reflection_of_upward_camera: true`, which explicitly permits the horizontal mirror `right_frame = -cross(view_direction_frame, up_frame)` and requires an upward `+Z` view direction. A view also must emit positive projected line length inside its declared bounds; partial crops remain valid when any positive segment remains in bounds.
...
For a candidate geometry edit, declare every edited object and its ancestor chain in the disposable scene, link those collections and objects into the active view layer, and verify that membership before reading evaluated transforms. An `evaluated_get` result is usable only after this membership check; otherwise its matrix may still describe the source scene's stale evaluation. Keep the original object names, parent links, transforms, material slots and source tags unless the edit explicitly changes one of them.

When an edit changes mesh topology, declare that change per object and compare the resulting mesh identity, material assignments and source tags separately from the preserved controls. Do not compute control displacement by subtracting arrays with different vertex counts or ordering. Compare the plan's corresponding original control vertices by explicit indices or labels, and report newly introduced vertices and faces as topology changes with their own checks.
...
Bind every comparison baseline to the authorizing current model SHA and verify the baseline inventory identity before comparing. Do not inherit an older worker's baseline path or embedded comparison as current; when the candidate inventory, schema and helper are valid, recompute the comparison offline against the correct retained baseline instead of rerunning Blender.
...
- Images: writable scalar/array settings, color-space settings, and custom tags. Object, mesh, and image tags preserve nested group values recursively, so editing a nested value changes that datablock's inventory row.
...
For an editable base-vertex check, an unparented, unconstrained object may still expose an identity cached `matrix_world` before evaluation. Require `obj.parent is None` and an empty constraint stack, use `matrix_basis` for the base vertices, and compare the accepted base topology by its stored vertex and face order. Parented, constrained, or evaluated geometry must be checked through the correctly activated scene/view layer and dependency graph; retain the distinction between base geometry and modifier-evaluated geometry.
...
For `draw_room.py` room sheets, shape-only illustrations whose geometry has no accepted metric or room-coordinate meaning can set `show_coordinates: false` and `legend: {"mode": "none"}` on that view. Coordinate ticks, grid, axis labels and the standard legend are then omitted. Ordinary room views keep the defaults; use `{"mode": "explicit", "items": ["...", "..."]}` when an illustration needs a scoped legend.
