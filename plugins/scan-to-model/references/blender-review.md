...
An evaluated object bound or historical full rectangle is only a screening extent; it does not prove that the current mesh covers that footprint. Ownership clipping can leave complementary stair-step fragments across adjoining room objects, so a zero-overlap or missing-geometry finding requires enumerating the current owners and intersecting their evaluated mesh polygons in the declared face frame before calling the geometry absent. Resolve the current owner of each fragment and retain that mapping in the comparison record.
