...
Its manifest separates per-view `nondrawable_objects` for exact selected point collapse from `omitted_objects` caused by projection, cuts or clipping; point collapse is a geometry diagnostic, not proof of physical absence, so coverage reviewers still inspect the pixels.
