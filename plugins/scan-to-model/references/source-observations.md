...
- Source-date claims and their provenance. Inspect original embedded image metadata before declaring dates unknown. Folder names, scan clock values, PDF metadata and EXIF dates retain their stated uncertainty; a timestamp without known epoch/units is not a calendar date. A camera clock is not construction chronology, and an EXIF offset is retained without converting it to UTC.
...
Assess chronology separately. Record camera timestamps verbatim with their device, time-zone/clock uncertainty and any conflicting metadata. Page order is not capture order; compute chronological order and minimum/maximum or group-gap endpoints from parsed timestamp values, retaining the source IDs and computed intervals rather than selecting endpoints from page order or narrative. A state difference can be supported even when its sequence or cause is unknown. Matching a historical installation does not prove that it survives behind present finishes, and matching a photographed enclosure does not by itself assign a current room, storey or model transform.
