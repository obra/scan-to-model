...
With `--coordinate-inspections`, each observation also gets one `coordinates.png`. Every authored coordinate appears as enlarged nearest-neighbor source pixels, with an unmarked panel beside an open-center locator panel and visible native/display coordinates. The evidence record gives each panel's output bounds and selects the nearest native pixel as `floor(coordinate + 0.5)` on each axis. A sheet uses at most eight rows before adding a column. This view makes selected pixels inspectable; it does not decide whether they depict the claimed item or whether the segment between coordinates stays on the intended face.
