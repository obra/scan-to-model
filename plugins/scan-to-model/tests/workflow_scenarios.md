...
On 2026-09-25, two fresh-context `gpt-5.6-luna` low-effort runs per variant handled E, F, I, K and L. Each read the coordinator and depth skills, evidence workflow, work packets and project records from an isolated snapshot, plus the scenario requests and the actual synthetic image for K. They received no assessment, proposed fix or other runs' answers. The baseline was `2619be3`; the first revision contained the judgment/provenance changes and initial model-selection guidance.

Both variants continued authorized interpolation, kept geometry-only scope, rejected unsupported registration/services claims, and opened the image to accept the floor-only selection and reject the wall/floor selection. Neither introduced a blanket human review gate. These cases expose instruction conflicts but did not demonstrate a baseline failure on every tightened rule.
