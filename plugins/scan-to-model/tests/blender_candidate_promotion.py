...
def run(save_mode):
    with tempfile.TemporaryDirectory(prefix="blender-candidate-promotion-") as temporary:
        root = Path(temporary)
        ...
        create_png(unpacked_path, (0.2, 0.4, 0.6, 1.0))
        create_png(packed_path, (0.8, 0.3, 0.1, 1.0))
        ...
        if save_mode == "default":
            ...
            bpy.ops.wm.save_as_mainfile(
                filepath=str(candidate_path), relative_remap=False
            )
        ...
        if not Path(promoted["Unpacked source"]["resolved"]).is_file():
            failures.append("unpacked source does not exist at its promoted resolution")
