...
No full private house has been replayed through this producer. It operates on an authored candidate and an explicit project configuration. Project-specific geometry, source registration, material samples and camera choices still require work. Photo baking is planar; browser lighting is approximate, free flight has no collisions, and metric acceptance remains separate from visual completion. New workflow scenarios E–H are recorded in [workflow_scenarios.md](workflow_scenarios.md) without claiming a separate fresh-context agent evaluation.
